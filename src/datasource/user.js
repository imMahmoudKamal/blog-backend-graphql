...
export class userDataSource extends DataSource {
  ...
  async register(input) {
    try {
      ...
      if (!isValid) throw new ApolloError(`The email is invalid email: ${input.email}`, 'GRAPHQL_VALIDATION_FAILED');
  ...
  async login(input) {
    try {
      ...
      if (isBlocked) throw new ApolloError('sorry you are blocked from login', 'UNAUTHENTICATED');
